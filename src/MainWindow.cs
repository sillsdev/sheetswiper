...
namespace SheetSwipe
{
    ...
    {
        private const string _busyMessage =
            "That file appears to already be open, probably in Excel. First close it, then drag it here.";
        ...
        private void UpdateConvertTabDisplay()
        {
            ...
            _dragStatus.MaximumSize = new Size(
                _convertPage.Width - 70,
                _dragStatus.MaximumSize.Height
            );
        }
        ...
        private void UpdateLinkForConvertAndSave()
        {
            ...
            _linkConvertAndSave.Visible =
                _linkConvertPrevious.Visible
            ...
            {
                _linkConvertAndSave.Text = String.Format(
                    "Convert {0} to {1} without asking any questions.",
                    Path.GetFileName(Settings1.Default.PreviousSheetPath),
                    Path.GetFileName(Settings1.Default.PreviousSavePath)
                );
            }
        ...
        private void UpdateLinkForConvertingPrevious()
        {
            ...
            if (doShowIt)
            {
                _linkConvertPrevious.Text = String.Format(
                    "Convert {0}",
                    Path.GetFileName(Settings1.Default.PreviousSheetPath)
                );
            }
        ...
        private bool Convert(string path)
        {
            ...
            {
                MessageBox.Show(
                    "That file appears to be open, probably in Excel. First close it, then try again.",
                    "Whoops",
            ...
            try
            {
                var output = Converter.Convert(path);
                if (Converter.ConverterErrors.ContainsValue(output))
                {
                    MessageBox.Show(output, "Whoops", MessageBoxButtons.OK);
                    return false;
                }
                _output.Text = output;
                _output.Select(0, 0);
            ...
            {
                MessageBox.Show(
                    "SheetSwiper was unable to read that file.\r\n" + error.Message,
                    "Whoops",
                    MessageBoxButtons.OK
                );
                return false;
        ...
        private void OnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                d.InitialDirectory = System.Environment.GetFolderPath(
            ...
            {
                MessageBox.Show(
                    "Whoa there... SheetSwiper cannot write spreadsheets, only text files. Your file has not been saved.",
                    "Whoops",
                    MessageBoxButtons.OK
                );
                return;
        ...
        private void Save(string path)
        {
            ...
            {
                MessageBox.Show(
                    "SheetSwiper was unable to save to '" + path + "'. \r\n" + error.Message,
                    "Whoops",
                    MessageBoxButtons.OK
                );
                return;
        ...
        private void _linkConvertPrevious_LinkClicked(
            object sender,
        ...
        private void _linkChooseFile_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            )
            {
                dialog.InitialDirectory = Path.GetDirectoryName(
                    Settings1.Default.PreviousSheetPath
                );
            }
            else
            {
                dialog.InitialDirectory = System.Environment.GetFolderPath(
                    Environment.SpecialFolder.MyDocuments
                );
            }
